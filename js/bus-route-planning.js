...
function applyUrlParameters() {
    ...
    if (params.city) {
        ...
        updateCurrentCityCode(params.city);
        saveCity(params.city);
        hasParams = true;
    }
...
function setupAutocomplete() {
    ...
    try {
        ...
        console.warn('自动完成功能暂时不可用:', error.message);
        return;
    }
...
function toggleApiConfig() {
    var content = document.getElementById('api-config-content');
    var icon = document.getElementById('api-toggle-icon');
    var text = document.getElementById('api-toggle-text');
...
function setupSettingsAutocomplete() {
    ...
    try {
        ...
        console.warn('设置自动完成功能暂时不可用:', error.message);
        return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
        function(position) {
            try {
                ...
                // 设置起点输入框的值
                document.getElementById('startinput').value = '当前位置';
                ...
                console.error('定位处理错误:', error);
                showMessage('定位信息处理失败');
            }
        },
        function(error) {
            var errorMessage = '';
        ...
        },
        {
            ...
            timeout: 10000,              // 10秒超时
            maximumAge: 300000           // 5分钟内的缓存位置也接受
        }
    );
}
...
function checkAmapAPILoaded() {
    if (typeof AMap === 'undefined') {
        throw new Error('高德地图API未加载，请先在设置中配置JS Web API Key和安全密钥');
    }
    return true;
}
...
function getCurrentCity() {
    return document.getElementById('cityinput').value.trim() || '北京市';
}
...
function searchRoute() {
    ...
    try {
        ...
        if (!window.AMAP_CONFIG || !window.AMAP_CONFIG.webApiKey) {
            throw new Error('请先在设置中配置Web API Key');
        }
    } catch (error) {
